...
public class AutoPathingScript
{
    ...
    public AutoPathingScript(string rootPath, object? config)
    {
        _config = config;
        _rootPath = rootPath;
        _autoPathingFile = new LimitedFile(UserPathProvider.PathingScriptsRoot);
    }
    ...
    /// 从已订阅的内容中获取文件
    /// </summary>
    /// <param name="path">在 `User\Scripts\Pathing` 目录下获取文件</param>
    ...
    /// <summary>
    /// 判断 Pathing 脚本目录下的路径是否存在
    /// </summary>
    /// <param name="subPath">相对于 `User\Scripts\Pathing` 的路径</param>
    /// <returns>存在返回 true，否则返回 false</returns>
    public bool IsExists(string subPath) => AutoPathingFile.IsExists(subPath);

    /// <summary>
    /// 判断 Pathing 脚本目录下的路径是否为文件
    /// </summary>
    /// <param name="subPath">相对于 `User\Scripts\Pathing` 的路径</param>
    ...
    /// <summary>
    /// 读取 Pathing 脚本目录下指定文件夹的内容（非递归方式）
    /// </summary>
    /// <param name="subPath">相对于 `User\Scripts\Pathing` 的子目录路径，默认为相对根目录</param>
    ...
    /// <summary>
    /// LimitedFile 实例，用于操作 Pathing 脚本目录
    /// </summary>
